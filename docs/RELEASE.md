...
- [ ] **SW version bump.** If this release changes ANY of:
  ...
  …bump `SW_VERSION` in `public/service-worker.js` to match the new release tag. `tests/View/ServiceWorkerStructureTest::test_sw_version_matches_release` enforces this — if you forget, CI fails.

  **Why this matters:** browsers detect SW updates via byte-comparison of `service-worker.js`. Without a bump on an asset-changing release, users keep the prior cache until the SW source happens to change for an unrelated reason — meaning a layout.twig change might not reach existing PWA installs for weeks. The version bump IS the deploy signal.
