...
**`jobs`** is karhu-queue's table — the worker pops `pending` rows, flips to `processing`, runs the handler, flips to `completed` or `failed`. `updated_at` tracks the last status transition (every transition bumps it as of karhu-queue v0.2.0); the `jobs:unstick` cron uses this to recover rows stuck in `processing` after a SIGKILL (decision #50 — the 10-min cron resets `processing` rows older than 5min back to `pending` so the worker picks them up again; idempotent via the `notification_dispatches` ledger for the push handler).
