...
final class ExerciseLogControllerTest extends AppTestCase
{
    ...
    // v0.8.4 — optional logged_on + JSON path for offline replay.

    public function test_post_accepts_optional_logged_on(): void
    {
        [$uid, $hid, $durId] = $this->signedIn();
        ...
        $row = $this->db->fetchOne('SELECT logged_on FROM exercise_log WHERE user_id = :uid', ['uid' => $uid]);
        self::assertNotNull($row);
    ...
    public function test_post_json_validation_reject_returns_400(): void
    {
        ...
        $r = $this->request('POST', '/health/log/exercise', [
            'exercise_id' => (string) $durId,
            'minutes' => '0',   // duration branch requires > 0
        ], headers: ['content-type' => 'application/x-www-form-urlencoded', 'accept' => 'application/json']);
