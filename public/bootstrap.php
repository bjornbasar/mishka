...
/**
 * Mishka Den — application bootstrap.
 *
 * Wires the Karhu\App instance: error handler, env validation, DB, Twig
 * (with brand global + CSRF extension), DI container wiring, middleware
 * (Session → SessionRevocationGuard → Csrf), route scan. Returns the
 * fully-configured App. The caller is responsible for `$app->run()`.
 *
 * Two callers:
 *   - public/index.php (production front controller, dispatches the
 *     current request)
 *   - tests/Smoke/BootstrapSmokeTest (CI invariant — asserts boot
 *     completes without throwing)
 *
 * Extracted from public/index.php in v0.6.16 to close the DOCS #56
 * follow-up: the smoke test now requires this file directly and asserts
 * the namespace-collision class of bug (latent in v0.6.13, hotfixed in
 * v0.6.15) cannot recur silently. See DOCS.md decision #57.
 */
...
// safeLoad() does NOT overwrite already-set $_ENV entries (immutable
// semantics) — the smoke test relies on this to inject test fixtures
